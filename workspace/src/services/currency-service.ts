...
export async function getCurrencyData(): Promise<CurrencyData> {
  ...
  if (!API_KEY || API_KEY === 'your_api_key_here') {
    console.warn("ExchangeRate API key is not set. Currency conversion will not work.");
  ...
  try {
    ...
    // In case of an error, return empty data but with all names so the dropdown still works
    const availableNames: { [key: string]: string } = {};
     Object.keys(currencyNames).forEach(code => {
        if (currencyNames[code]) {
            availableNames[code] = currencyNames[code];
